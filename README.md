...
Gate 5C originally migrated Wardrobe Check and Material tools onto the same wardrobe/inventory boundaries while both were still classic scripts. Material still uses the classic `WardrobeDomain` / `InventoryDomain` bridge, while Wardrobe Check has since moved to direct ESM imports in Gate 7B. Gate 5D consolidates duplicated inventory behavior into the shared inventory domain and centralizes localStorage/cookie fallback in `readBrowser()` / `writeBrowser()`. The public classic `MyClothes()` wrappers remain for callers such as `nikki.js` and `sharewardrobe.js`, but no longer own persistence logic. ESLint covers the migrated legacy scripts with explicit cross-script globals so accidental globals fail CI.
...
Gate 6D removes Material's remaining jQuery dependency and the unused Knockout load. `material.html` now uses the shared native DOM facade plus `src/legacy/material-actions.js`; generated Material links/selects use delegated `data-material-action` / `data-material-change` bindings instead of inline handlers. `material.js` is included in ESLint, and the retired `jquery.js` / `knockout.js` vendor files are removed from the repository. After Gate 7B, the classic `WardrobeDomain` / `InventoryDomain` browser bridges remain intentionally constrained to the two synchronous classic consumers (`model.js` and `material_model.js`). Removing those bridges safely requires converting those model chains to ESM or an equivalent synchronous module boundary.
