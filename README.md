...
Gate 5C migrates Wardrobe Check and Material tools onto the same wardrobe/inventory boundaries. Gate 5D then consolidates their duplicated inventory objects into `InventoryDomain.createInventory()` and centralizes localStorage/cookie fallback in `readBrowser()` / `writeBrowser()`. The public classic `MyClothes()` wrappers remain intentionally for callers such as `nikki.js` and `sharewardrobe.js`, but no longer own persistence logic. ESLint now covers the migrated legacy scripts (`model.js`, BigUse, Material model and Wardrobe Check) with explicit cross-script globals, so accidental globals fail CI while the remaining classic-script API stays compatible. Full ESM/global removal is deferred to Gate 6.
