...
Gate 7D migrates the Main Matcher runtime to a single browser-native `main.mjs` entry. The model imports wardrobe/inventory ESM APIs directly, UI/controller/strategy/support files are explicit modules, and the pure `accMul()` helper moves from controller ownership into `model.mjs` to avoid a model-to-controller cycle. Main page events use an explicit `MainActions.register()` registry rather than leaking module functions back onto `window`. Because BigUse historically depended on the entire classic Main script chain and its late-binding overrides, Gate 7D keeps the original root `.js` chain unchanged for BigUse instead of silently rewriting BigUse in the same gate. `index.html` no longer loads those classic scripts, while `biguse.html` remains their sole active page consumer. Main Matcher, Material, and Wardrobe Check are bridge-free; BigUse compatibility is the sole remaining wardrobe/inventory classic-bridge consumer and is the bounded target for Gate 7E.
