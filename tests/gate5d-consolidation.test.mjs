...
import * as domain from '../src/domain/inventory/index.mjs';
...
test('shared inventory factory preserves matcher/material semantics', () => {
  ...
  assert.deepEqual(inv.mine, { 1: ['001'], 8: ['010'] });
...
test('browser storage helpers preserve localStorage precedence and cookie fallback', () => {
  ...
  assert.deepEqual(domain.readBrowser(storage, { cookie: 'mine2=cookie' }), { current: '1:001|', legacy: 'legacy' });
  ...
  assert.deepEqual(domain.readBrowser(null, doc), { current: '1:001|', legacy: 'OldName' });
  const outDoc = { cookie: '' };
  domain.writeBrowser(null, outDoc, '1:001|');
  assert.match(outDoc.cookie, /^mine2=1%3A001%7C; expires=/);
});

test('inventory consumers delegate to shared ESM boundaries and are lint-covered', () => {
  ...
  for (const file of ['model.mjs', 'main.mjs', 'biguse_model.mjs', 'biguse_ui.mjs', 'material_model.mjs', 'material.mjs', 'wardrobechk.mjs']) {
