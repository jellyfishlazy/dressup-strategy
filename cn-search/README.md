...
- **不需要** `cn-search/node_modules/`（此目錄不應存在）
...
建索引腳本需要 `opencc-js`，由共用的 [`my-projects/node_modules`](../../my-projects/node_modules) 提供。

1. **安裝共用依賴**（僅需在 `my-projects` 執行一次）：
...
2. **產生索引**（陸服或台服 `../data/wardrobe.js` 更新後請重跑）：
   ...
   **一鍵重建**：雙擊 [`重建搜尋索引.bat`](重建搜尋索引.bat)（會自動設定共用 `node_modules` 並執行建索引）。

   或手動執行：

   ```bash
   cd ../dressup-strategy-main/cn-search
...
- **自訂共用路徑**：可設環境變數 `CN_SEARCH_NODE_MODULES` 指向其他 `node_modules` 目錄。
...
| `../../my-projects/node_modules` | 共用 npm 套件（opencc-js） |
